...
static void vdev_setup_vrect(VDEV_COMMON_CTXT *vdev) {
    int rw = vdev->rrect.right - vdev->rrect.left, rh = vdev->rrect.bottom - vdev->rrect.top, vw,
        vh;
...
#ifdef WIN32
    switch (type) {
        case VDEV_RENDER_TYPE_GDI:
            c = (VDEV_COMMON_CTXT *)vdev_gdi_create(surface, bufnum);
            break;
        case VDEV_RENDER_TYPE_D3D:
            c = (VDEV_COMMON_CTXT *)vdev_d3d_create(surface, bufnum);
            break;
    }
    if (!c)
        return NULL;
        ...
        c->hoverbmp =
            CreateDIBSection(c->hoverlay, &bmpinfo, DIB_RGB_COLORS, &c->poverlay, NULL, 0);
...
void vdev_avsync_and_complete(void *ctxt) {
    ...
    } else if (!c->cmnvars->apktn && !c->cmnvars->vpktn &&
               ++c->completed_counter == COMPLETED_COUNTER) {
        c->status |= VDEV_COMPLETED;
    ...
    scdiff = (int)(sysclock - c->cmnvars->vpts -
                   c->tickavdiff);  // diff between system clock and video pts
    avdiff = (int)(c->cmnvars->apts - c->cmnvars->vpts -
                   c->tickavdiff);  // diff between audio and video pts
    avdiff =
        c->cmnvars->apts <= 0 ? scdiff : avdiff;  // if apts is invalid, sync video to system clock
    ...
    if (c->ticksleep > 0 && c->cmnvars->init_params->avts_syncmode != AVSYNC_MODE_LIVE_SYNC0) {
        av_usleep(c->ticksleep * 1000);
    }
...
void vdev_win32_render_overlay(void *ctxt, HDC hdc, int erase) {
    ...
    if (hdc == NULL) return;
    if (c->overlay_rects[0].type == 0) {
        c->status |= VDEV_CLEAR;
        return;
    }
    for (i = 0; i < sizeof(c->overlay_rects) / sizeof(c->overlay_rects[0]) &&
                c->overlay_rects[i].type != 0;
         i++) {
        if (erase) {
        ...
        func.SourceConstantAlpha =
            c->overlay_rects[i].type == OVERLAY_COPY_BITBLT ? 255 : c->overlay_rects[i].alpha;
        func.AlphaFormat = c->overlay_rects[i].type == OVERLAY_PIXEL_ALPHA ? AC_SRC_ALPHA : 0;
        if (c->overlay_rects[i].type == OVERLAY_TRANSPARENT) {
            TransparentBlt(hdc, c->overlay_rects[i].dstx, c->overlay_rects[i].dsty,
                           c->overlay_rects[i].dstw, c->overlay_rects[i].dsth, c->hoverlay,
                           c->overlay_rects[i].srcx, c->overlay_rects[i].srcy,
                           c->overlay_rects[i].srcw, c->overlay_rects[i].srch,
                           c->overlay_rects[i].transparent);
        } else {
            AlphaBlend(hdc, c->overlay_rects[i].dstx, c->overlay_rects[i].dsty,
                       c->overlay_rects[i].dstw, c->overlay_rects[i].dsth, c->hoverlay,
                       c->overlay_rects[i].srcx, c->overlay_rects[i].srcy, c->overlay_rects[i].srcw,
                       c->overlay_rects[i].srch, func);
        }
    }
}
