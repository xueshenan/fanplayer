...
// ����ʵ��
void *render_open(int adevtype, int vdevtype, void *surface, struct AVRational frate, int w, int h,
                  CMNVARS *cmnvars) {
    ...
    render->adev_buf_avail = render->adev_buf_size =
        (int)((double)ADEV_SAMPLE_RATE / (h ? 60 : 46) + 0.5) * 4;
    ...
    if (1) {
        RECT rect;
        GetClientRect((HWND)surface, &rect);
...
static int render_audio_soundtouch(RENDER *render, AVFrame *audio) {
    ...
    num_samp = swr_convert(render->swr_context, (uint8_t **)&out, 1024 / 2,
    ...
    do {
        num_st = soundtouch_receiveSamples_i16(render->stcontext, (int16_t *)render->adev_buf_cur,
        ...
        if (render->adev_buf_avail == 0) {
            ...
            if (render->veffect_type != VISUAL_EFFECT_DISABLE) {
                veffect_render(render->veffect_context, render->veffect_x, render->veffect_y,
                               render->veffect_w, render->veffect_h, render->veffect_type,
                               render->adev);
            }
#endif
            swvol_scaler_run((int16_t *)render->adev_buf_data,
                             render->adev_buf_size / sizeof(int16_t),
                             render->vol_scaler[render->vol_curvol]);
            audio->pts +=
                5 * render->cur_speed_value * render->adev_buf_size / (2 * ADEV_SAMPLE_RATE);
...
static int render_audio_swresample(RENDER *render, AVFrame *audio) {
    ...
    num_samp = swr_convert(render->swr_context, (uint8_t **)&render->adev_buf_cur,
...
#endif
        swvol_scaler_run((int16_t *)render->adev_buf_data, render->adev_buf_size / sizeof(int16_t),
                         render->vol_scaler[render->vol_curvol]);
...
void render_audio(void *hrender, AVFrame *audio) {
    ...
    if (!render || (render->cmnvars->init_params->avts_syncmode != AVSYNC_MODE_FILE &&
    ...
    do {
        if (render->swr_src_format != audio->format ||
            ...
            render->cur_speed_type = render->new_speed_type;
            render->cur_speed_value = render->new_speed_value;
            samprate = render->cur_speed_type
                         ? ADEV_SAMPLE_RATE
            ...
            render->swr_context = swr_alloc_set_opts(
                NULL, AV_CH_LAYOUT_STEREO, AV_SAMPLE_FMT_S16, samprate, render->swr_src_chlayout,
                render->swr_src_format, render->swr_src_samprate, 0, NULL);
            swr_init(render->swr_context);
#if CONFIG_ENABLE_SOUNDTOUCH
            if (render->cur_speed_type)
                soundtouch_setTempo(render->stcontext, (float)render->cur_speed_value / 100);
#endif
        }
#if CONFIG_ENABLE_SOUNDTOUCH
...
static void render_setup_srcrect(RENDER *render, AVFrame *video, AVFrame *srcpic) {
    ...
    switch (video->format) {
        case AV_PIX_FMT_YUV420P:
            srcpic->data[0] +=
                render->cur_src_rect.top * video->linesize[0] + render->cur_src_rect.left;
...
void render_video(void *hrender, AVFrame *video) {
    ...
    if (render->status & RENDER_DEFINITION_EVAL) {
        render->definitionval =
            definition_evaluation(video->data[0], video->width, video->height, video->linesize[0]);
    ...
    do {
        ...
        if (dstpic.data[0] && srcpic.format != -1 && srcpic.pts != -1) {
            if (render->sws_src_pixfmt != srcpic.format || render->sws_src_width != srcpic.width ||
                render->sws_src_height != srcpic.height || render->sws_dst_pixfmt != vdev->pixfmt ||
                ...
                render->sws_context = sws_getContext(
                    render->sws_src_width, render->sws_src_height, render->sws_src_pixfmt,
                    render->sws_dst_width, render->sws_dst_height, render->sws_dst_pixfmt,
                    render->cmnvars->init_params->swscale_type, 0, 0, 0);
                player_log(LOG_TYPE_DEBUG,
                           "src resolution %dx%d, format:%d, dest resolution:%dx%d, format=%d",
                           render->sws_src_width, render->sws_src_height, render->sws_src_pixfmt,
                           render->sws_dst_width, render->sws_dst_height, render->sws_dst_pixfmt);
            }
            if (render->sws_context) {
                sws_scale(render->sws_context, (const uint8_t **)srcpic.data, srcpic.linesize, 0,
                          render->sws_src_height, dstpic.data, dstpic.linesize);
            }
        }
...
void render_setparam(void *hrender, int id, void *param) {
    ...
    switch (id) {
        ...
        case PARAM_VISUAL_EFFECT:
            ...
            if (render->veffect_type == VISUAL_EFFECT_DISABLE) {
                veffect_render(render->veffect_context, render->veffect_x, render->veffect_y,
                               render->veffect_w, render->veffect_h, VISUAL_EFFECT_DISABLE,
                               render->adev);
            }
            break;
#endif
    ...
    switch (id) {
        case PARAM_MEDIA_POSITION:
            if (vdev && vdev->status & VDEV_COMPLETED) {
                ...
                *(int64_t *)param =
                    render->cmnvars->apts != -1 ? render->cmnvars->apts : render->cmnvars->vpts;
            }
            break;
        case PARAM_AUDIO_VOLUME:
            *(int *)param = render->vol_curvol - render->vol_zerodb;
            break;
        case PARAM_PLAY_SPEED_VALUE:
